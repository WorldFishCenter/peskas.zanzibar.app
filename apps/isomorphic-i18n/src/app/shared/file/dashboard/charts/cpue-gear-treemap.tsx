...
import { districtsAtom, selectedTimeRangeAtom, TIME_RANGES } from "@/app/components/filter-selector";
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white p-4 border border-gray-200 rounded-lg shadow-lg">
        <p className="text-sm font-medium text-gray-600 mb-2">{data.gear}</p>
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            />
            <p className="text-sm">
              <span className="font-medium">{t("text-average-cpue") || "Average CPUE"}:</span>{" "}
              <span className="font-semibold">
                {isValidValue ? formatNumber(data.avg_cpue) : t("text-na")}
              </span>
              {isValidValue && (
                <span className="text-gray-500 ml-1">kg/fisher/day</span>
              )}
            </p>
          </div>
          <p className="text-sm text-gray-600">
            <span className="font-medium">{t("text-districts") || "Districts"}:</span>{" "}
            <span className="font-semibold">{data.district_count}</span>
          </p>
          <p className="text-sm text-gray-600">
            <span className="font-medium">{t("text-records") || "Records"}:</span>{" "}
            <span className="font-semibold">{data.total_records}</span>
          </p>
          <p className="text-xs text-gray-500 italic">
            {t("text-cpue-treemap-explanation") || "Box size represents the average CPUE value"}
          </p>
        </div>
      </div>
    );
